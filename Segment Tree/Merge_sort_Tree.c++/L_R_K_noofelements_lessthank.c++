...
//No. of elements in the range L to R which are strictly smaller than K.
#include<bits/stdc++.h>
using namespace std;
//at each node we will store the elements in that range in the sorted order.
class Merge_Sort_Tree{
    int MSSize, n;
    vector<set<int>> *MSVec;
    public:
    Merge_Sort_Tree(vector<int>& arr){
        ...
        if(log2(n) == power) power++;
            ...
            power += 2;
        MSSize = 1<<power;
        MSSize = new vector<set<int>>(MSVec);
    ...
    void BuildTree(int si, int ss, int se,vector<int>& arr){
        ...
        (*MSVec)[si] = Merge(2*si+1, 2*si+2);
    }
    set<int> Merge(int l, int r){
        set<int> temp;
        std::merge((*MSVec)[l].begin(), (*MSVec)[l].end(), (*MSVec)[r].begin(), (*MSVec)[r].end(), std::inserter(temp, temp.begin()));
        return temp;
    }
...
int main(){
    ...
    for (int i = 0; i < qn; i++)
    {
        ...
        else if (Query[i][0] == 2)
        {
            ...
            cout << "RangeUpdate("<< Query[i][1] << ", " << Query[i][2] << ")   :   " << Seg.RangeQuery(0, 0, n - 1, Query[i][1], Query[i][2])<< endl;
        }
    }
    return 0;
}